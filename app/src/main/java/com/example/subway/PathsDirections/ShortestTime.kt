...
fun dijkstraTime(context: Context?, startStationId: Int, endStationId: Int): PathInfo? {
    ...
    if (startStation != null && endStation != null) {
        ...
        for (i in 1 until path.size) {
            ...
            if (i < path.size - 1) {
                ...
                // 각 엣지의 시간을 더함
                totalCost += path[i].edges.find { it.destination == nextStation }?.cost ?: 0
            }
            count++
        }
        ...
        return PathInfo(
            ...
            totalTime = distances[endStation]?.let { it } ?: 0 // 총 시간
        )
...
fun findPrevAndNextTime(path: List<Node>, startStation: Node, endStation: Node, currentStation: Node): Pair<Node?, Node?> {
